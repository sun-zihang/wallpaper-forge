...
- **批量**：拖拽添加、进度、取消、失败详情；一键「重试失败项」只重跑失败文件；完成对话框可直接打开输出文件夹，双击列表行定位到真实产物；全选/反选与「已选中 N/M」提示；输出默认到源文件旁 `converted/` 子目录，可切换统一目录；默认永不覆盖，勾选「覆盖原文件」后允许就地覆盖源文件与已存在输出（覆盖源前需确认）；快捷键：Ctrl+O 添加文件、Ctrl+Shift+O 添加文件夹、Ctrl+Enter 开始处理、Esc 取消任务、Ctrl+A 全选
